...
public class ChatService {
    ...
    public void sendDirectMessage(String roomId, DirectMessageRequest directMessageRequest, Long senderId) {
        ...
        // ✅ [변경] 수신자와 발신자의 개인 큐 목적지를 직접 생성하여 메시지 전송
        String recipientQueue = "/queue/private." + recipient.getId();
        String senderQueue = "/queue/private." + sender.getId();

        messagingTemplate.convertAndSend(recipientQueue, responseDto);
        messagingTemplate.convertAndSend(senderQueue, responseDto); // 발신자에게도 자신의 메시지를 다시 보내줌 (UI 동기화용)
    ...
    private void sendNotification(ChatMessage message, UserDto sender, Long recipientId, String roomId) {
        ...
        String notificationQueue = "/queue/notifications." + recipientId;
        messagingTemplate.convertAndSend(notificationQueue, notification);
    }

...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        // STOMP CONNECT 요청일 때, Spring Security가 이미 인증한 사용자인지 확인
        if (StompCommand.CONNECT.equals(accessor.getCommand())) {
            log.info("✅ STOMP CONNECT 요청 처리 시작");

            // 👇 [핵심 변경]
            // 티켓을 검증하는 대신, WebSocket 세션에 이미 Principal(인증 정보)이 있는지 확인합니다.
            // 이 Principal은 Spring Session과 Handshake Interceptor를 통해 HTTP 세션으로부터 자동으로 주입됩니다.
            if (accessor.getUser() == null) {
                log.error("❌ STOMP CONNECT 에러: 인증된 사용자를 찾을 수 없습니다. 세션이 없거나 만료되었을 수 있습니다.");
                // 필요하다면 여기서 예외를 던져 연결을 거부할 수 있습니다.
                // throw new AccessDeniedException("인증 정보가 없습니다.");
                return null; // 또는 null을 반환하여 메시지 전송 중단
            }

            log.info("✅ WebSocket 세션 인증 성공. 사용자: {}", accessor.getUser().getName());
        }
